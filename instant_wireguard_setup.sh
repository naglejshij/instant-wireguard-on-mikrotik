...
echo "/ip/firewall/filter/add chain=input action=accept dst-port=$listen_port dst-address=$server_ip protocol=udp place-before=2 comment=\"$wg_interface_name inbound\"" >> $mt_config
